...
def create_sber_phone_stealth(data: Dict) -> Optional[bytes]:
    ...
    # No charset refusals — any FIO must emit (font-patch / soft-fail only).
    ...
    try:
        ...
        logger.warning("Sber phone: unreadable — ship if PDF")
        return result if result.startswith(b"%PDF-") else None
    if _pad_after_et_burned(stream):
        logger.warning("Sber phone: pad-after-ET — ship")
    if not _face_has_exact_fields(text, prepared):
        logger.error("Sber phone: face field mismatch — ship anyway")
