"""
Менеджер шаблонов Ozon Bank - автоматически выбирает шаблон с нужными буквами.
Работает так же как template_manager для ВТБ.
"""
...
CACHE_FILE = "templates/ozon_templates_cache.json"


def extract_available_chars_from_pdf(pdf_path: str) -> Set[str]:
    """Извлекает доступные символы из ToUnicode потока PDF."""
    ...
    try:
        ...
        for xref in range(1, doc.xref_length()):
            try:
                ...
                # Парсим bfrange и bfchar
                content = stream.decode('latin-1', errors='ignore')
                
                # bfrange: <CID_start> <CID_end> <Unicode_start>
                ...
                # bfchar: <CID> <Unicode>
                for match in re.finditer(r'<([0-9A-Fa-f]+)>\s*<([0-9A-Fa-f]+)>\s*$', content, re.MULTILINE):
                    try:
...
def is_ozon_pdf(pdf_path: str) -> bool:
    """Проверяет является ли PDF чеком Озон Банка."""
    try:
        doc = fitz.open(pdf_path)
        ...
        if metadata:
            ...
            # Ozon чеки генерируются Chromium/Skia
            if 'skia' in producer or 'chromium' in creator:
                # Дополнительно проверяем текст
                page = doc[0]
    ...
    if 'ozon' in filename or 'озон' in filename:
        return True
...
def scan_templates() -> Dict[str, Set[str]]:
    """Сканирует все Ozon PDF в папке templates и возвращает доступные символы для каждого."""
    ...
    for filename in os.listdir(TEMPLATES_DIR):
        if filename.endswith('.pdf') and not filename.startswith('test'):
            ...
            # Проверяем что это Ozon PDF
            if not is_ozon_pdf(pdf_path):
                continue
            
            logger.info(f"Сканирую Ozon: {filename}")
            available = extract_available_chars_from_pdf(pdf_path)
            
            if available:
                templates[pdf_path] = available
                cyrillic = [c for c in available if '\u0400' <= c <= '\u04FF']
                logger.info(f"  Найдено {len(available)} символов, кириллица: {len(cyrillic)}")
...
def save_cache(templates: Dict[str, Set[str]]):
    ...
    logger.info(f"Кэш Ozon сохранён: {CACHE_FILE}")
...
def get_templates_info(force_rescan: bool = False) -> Dict[str, Set[str]]:
    ...
    # Сканируем заново
    logger.info("Сканирование Ozon шаблонов...")
...
def find_template_for_text(text: str, templates: Dict[str, Set[str]] = None) -> Optional[str]:
    ...
    if not templates:
        logger.error("Нет доступных Ozon шаблонов!")
        return None
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("МЕНЕДЖЕР ШАБЛОНОВ OZON BANK")
    print("="*60)
    ...
    print(f"\nНайдено Ozon шаблонов: {len(templates)}")
